...
if (is.null(CCG_code) & is.null(practice_code)) {variablesegment <- str_c("org_type=ccg")}

if (!is.null(CCG_code) & is.null(practice_code)) {variablesegment <- str_c("org_type=ccg&q=", CCG_code)}
  
if (is.null(CCG_code) & !is.null(practice_code)) {variablesegment <- str_c("q=", practice_code)}

if (!is.null(CCG_code) & !is.null(practice_code)) {variablesegment <- str_c("q=", CCG_code, ",", practice_code)}

str_c("https://openprescribing.net/api/1.0/", "org_location/?", variablesegment) %>%
readLines() %>%
rjson::fromJSON()
}
```